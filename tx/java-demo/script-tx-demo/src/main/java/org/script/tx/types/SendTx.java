...
public final class SendTx implements Tx {
    ...
    public SendTx(byte[] senderAddr, byte[] receiverAddr, BigInteger SCPTWei, BigInteger SPAYWei, 
        BigInteger feeInSPAYWei, long senderSequence) {
        this.fee = new Coins(BigInteger.valueOf(0), feeInSPAYWei);

        TxInput txInput = new TxInput(senderAddr, SCPTWei, SPAYWei.add(feeInSPAYWei), senderSequence);
        this.inputs = new TxInput[]{txInput};

        TxOutput txOutput = new TxOutput(receiverAddr, SCPTWei, SPAYWei);
