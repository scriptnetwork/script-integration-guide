...
export class SendTx extends Tx{
    constructor(senderAddr, receiverAddr, SCPTWei, SPAYWei, feeInSPAYWei, senderSequence){
        super();

        this.fee = new Coins(new BigNumber(0), feeInSPAYWei);

        let txInput = new TxInput(senderAddr, SCPTWei, SPAYWei.plus(feeInSPAYWei), senderSequence);
        this.inputs = [txInput];

        let txOutput = new TxOutput(receiverAddr, SCPTWei, SPAYWei);
