...
it('get account', async () => {
  ...
  expect(scptwei).toBeDefined()
  expect(spaywei).toBeDefined()
...
it('get account with shema', async () => {
  const coins = {
    scptwei: 'coins.scptwei',
    spaywei: 'coins.spaywei'
  }
...
function expectCombined (actual) {
  ...
  expect(chain_id).toBe('scriptnet')
...
it('get transaction', async () => {
  ...
  expect(proposer.coins.scptwei).toBeDefined()
  expect(proposer.coins.spaywei).toBeDefined()
  expect(proposer.sequence).toBeDefined()
  expect(proposer.signature).toBeDefined()

  expect(outputs[0].address).toBeDefined()
  expect(outputs[0].coins.scptwei).toBeDefined()
  expect(outputs[0].coins.spaywei).toBeDefined()
})
